...
internal class BarnOppslag(
    ...
    internal suspend fun barn(ident: Ident): Set<Barn>? {

        val tpsBarn = tpsProxyV1Gateway.barn(ident = ident) ?: return null

        return tpsBarn
            .filter { it.dødsdato == null }
            .map {
                Barn(tpsBarn = it, aktørId = aktoerRegisterV1Gateway.aktørId(ident = ident))
            }.toSet()
    }
}
